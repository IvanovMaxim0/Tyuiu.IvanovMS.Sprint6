using Tyuiu.IvanovMS.Sprint6.Task5.V16.Lib;
namespace Tyuiu.IvanovMS.Sprint6.Task5.V16
{
    public partial class FormMain : Form
    {
        ...
        private void UpdateFileStatus()
        {
            ...
            {
                labelFileStatus_IMS.Text = $"✅ Файл найден: {Path.GetFileName(inputFilePath)}";
                labelFileStatus_IMS.ForeColor = Color.Green;
                buttonLoad_IMS.Enabled = true;
                buttonLoad_IMS.BackColor = Color.LimeGreen;

                // Показываем информацию о файле
                FileInfo fileInfo = new FileInfo(inputFilePath);
                textBoxInfo_IMS.Text = $"📄 Имя файла: {Path.GetFileName(inputFilePath)}\n" +
            ...
            {
                labelFileStatus_IMS.Text = $"❌ Файл не найден: {Path.GetFileName(inputFilePath)}";
                labelFileStatus_IMS.ForeColor = Color.Red;
                buttonLoad_IMS.Enabled = false;
                buttonLoad_IMS.BackColor = Color.LightGray;
                textBoxInfo_IMS.Text = $"Файл '{Path.GetFileName(inputFilePath)}' не найден.\n" +
        ...
        private void buttonLoad_IMS_Click(object sender, EventArgs e)
        {
            try
            {
                // Загружаем данные
        ...
        {
            textBoxData_IMS.Clear();

            if (loadedData == null || loadedData.Length == 0)
            {
                textBoxData_IMS.Text = "Нет данных для отображения";
                return;
            }

            textBoxData_IMS.Text = "📋 ЗАГРУЖЕННЫЕ ДАННЫЕ (округлены до 3 знаков):\n";
            textBoxData_IMS.AppendText(new string('═', 50) + "\n");
            textBoxData_IMS.AppendText("№     Значение     Статус\n");
            textBoxData_IMS.AppendText(new string('═', 50) + "\n");
            ...
            for (int i = 0; i < loadedData.Length; i++)
            {
                ...
                textBoxData_IMS.AppendText($"{i + 1,3}: {loadedData[i],10:F3}     {status}\n");
            ...
            textBoxData_IMS.AppendText(new string('═', 50) + "\n");
            textBoxData_IMS.AppendText($"📊 Всего элементов: {loadedData.Length}\n");
        }

        private void DisplayStatistics()
        {
            textBoxStats_IMS.Clear();

            if (loadedData == null || loadedData.Length == 0)
            {
                textBoxStats_IMS.Text = "Нет данных для статистики";
                return;
            }

            textBoxStats_IMS.Text = "📈 СТАТИСТИЧЕСКИЙ АНАЛИЗ ДАННЫХ:\n";
            textBoxStats_IMS.AppendText(new string('═', 50) + "\n");
            textBoxStats_IMS.AppendText($"🔢 Количество элементов: {loadedData.Length}\n");
            textBoxStats_IMS.AppendText($"💰 Сумма всех значений: {ds.GetSum(loadedData):F3}\n");
            textBoxStats_IMS.AppendText($"📊 Среднее арифметическое: {ds.GetAverage(loadedData):F3}\n");
            textBoxStats_IMS.AppendText($"⬆️ Максимальное значение: {ds.GetMax(loadedData):F3}\n");
            textBoxStats_IMS.AppendText($"⬇️ Минимальное значение: {ds.GetMin(loadedData):F3}\n");
            textBoxStats_IMS.AppendText($"➕ Положительных чисел: {ds.GetPositiveCount(loadedData)}\n");
            textBoxStats_IMS.AppendText($"➖ Отрицательных чисел: {ds.GetNegativeCount(loadedData)}\n");
            textBoxStats_IMS.AppendText($"⚪ Нулевых значений: {loadedData.Count(x => Math.Abs(x) < 0.0001)}\n");
            textBoxStats_IMS.AppendText(new string('═', 50) + "\n");
            ...
            if (loadedData.Length > 1)
            {
                ...
                textBoxStats_IMS.AppendText($"📏 Диапазон значений: {range:F3}\n");

                double variance = loadedData.Select(x => Math.Pow(x - ds.GetAverage(loadedData), 2)).Sum() / (loadedData.Length - 1);
                double stdDev = Math.Sqrt(variance);
                textBoxStats_IMS.AppendText($"📐 Дисперсия: {Math.Round(variance, 3):F3}\n");
                textBoxStats_IMS.AppendText($"📏 Стандартное отклонение: {Math.Round(stdDev, 3):F3}\n");
            }
        }

        private void buttonSave_IMS_Click(object sender, EventArgs e)
        {
            if (loadedData == null || loadedData.Length == 0)
            {
...
Группа: АСОиУб-23-2";
        ...
        private void buttonOpenFolder_IMS_Click(object sender, EventArgs e)
        {
            try
            {
